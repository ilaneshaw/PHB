...
birdList <- sort(c("AMRO", "BARS", "BBWA", "BBWO", "BCCH", "CAWA", "CHSP", "DEJU", "EVGR", "GRAJ", "HAWO", "OSFL", "OVEN", "RBNU", "RCKI", "RUBL", "SWTH", "WTSP", "YBSA", "YEWA")) 
...
## grepl function searches for all items in the filesToDownload that are on birdList & stores their names in rastersforBirdList
   ...
   birdRasters <- lapply(X = birdRasters, FUN = function(RasterLayer) {
    ...
    print(paste0(names(RasterLayer), " postprocessed"))
    return(proRaster)
  }) 
...
birdPreds1DSingleFrame <- rbind(birdPreds1D$BARS, birdPreds1D$GRAJ, birdPreds1D$OVEN, birdPreds1D$RCKI)
...
birdCoverDensity <- birdDatasets$RCKI
...
names(class.labs) <- c("0_0", "0_1", "0_2", "0_3", "0_4", "0_5", "0_6", "1_1", "1_2", "1_3", "1_4", "1_5", "1_6", "1_7")
  ...
  geom_density() +
    ...
    theme_classic() +
  ggtitle("Density plots for RCKI") +
...
#make dataframe of birds you want to include
dataFrameForGraph <- rbind(matrixAsTables$BARS, matrixAsTables$CAWA, matrixAsTables$OVEN, matrixAsTables$RCKI)
